...
const PendingVerification = () => {
      ...
      try {
        ...
        if (!wallet) {
          try {
            ...
            // Continue anyway to load documents
          }
        }
        ...
        setError("Failed to initialize application properly.");
      } finally {
        setLoading(false);
      }
    };
  ...
  // Handle document review - FIXED VERSION
  const handleReviewDocument = (docId) => {
    try {
      ...
      // Check verifier status before allowing review (only in production)
      if (!isVerifier && process.env.NODE_ENV === 'production') {
        toast.error("You don't have verifier permissions");
        return;
      }
                ...
                <table className="min-w-full divide-y divide-gray-300">
                  ...
                  <tbody className="divide-y divide-gray-200 bg-white">
                    {filteredDocuments.map((doc, index) => {
                      // Generate a truly unique key for each row using array index as a last resort
                      // This ensures uniqueness even if two documents have identical properties
                      const rowKey = doc.id !== undefined && doc.id !== null 
                        ? `id-${doc.id}` 
                        : `doc-${index}-${doc.documentHash ? doc.documentHash.substring(0, 8) : 'unknown'}-${Date.now()}`;
                      
                      // Make sure we have a valid clientAddress before trying to use substring
                      ...
                      return (
                        <tr key={rowKey}>
                          ...
                          <td className="relative whitespace-nowrap py-4 pl-3 pr-4 text-right text-sm font-medium sm:pr-6">
                            <button
                              onClick={() => {
                                // Only call handleReviewDocument if we have a valid ID
                                if (doc.id !== undefined && doc.id !== null) {
                                  handleReviewDocument(doc.id);
                                } else {
                                  toast.error("This document has an invalid ID and cannot be reviewed");
                                }
                              }}
                              className="text-blue-600 hover:text-blue-900 flex items-center justify-end"
                              disabled={doc.id === undefined || doc.id === null}
                            >
